...
## النشر الآمن عبر Tailscale (اختياري)
...
عند فتح التطبيق لأجهزة أخرى عبر الشبكة، **فعّل حماية بسيطة (Basic Auth)** بإضافة متغيرين في `.env`:

```
APP_USERNAME=اسم_مستخدم_من_اختيارك
APP_PASSWORD=كلمة_سر_قوية
```

- لو **الاثنان فارغان** (الوضع الافتراضي) → التطبيق يعمل محليًا **بدون أي تسجيل دخول** كما هو.
- بمجرد ضبط `APP_USERNAME`، أي طلب للسيرفر يطلب اسم مستخدم/كلمة سر (نافذة تسجيل دخول المتصفح). أعد تشغيل السيرفر بعد التعديل.

> هذه حماية بسيطة مناسبة للاستخدام الشخصي خلف شبكة Tailscale الخاصة — وليست بديلًا عن TLS/HTTPS على شبكة عامة.
